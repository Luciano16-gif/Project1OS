...
public class MemoryManager {
  ...
  private final Compare.Comparator<PCB> mostCriticalComparator = (left,
      right) -> -leastCriticalComparator.compare(left, right);
  ...
  public void reset() {
    readySuspended.clear();
    blockedSuspended.clear();
  }
  ...
  private int getResidentCount() {
    int ready = os.isFifoAlgorithmInternal() ? os.getReadyQueueFIFO().size() : os.getReadyListSorted().size();
  ...
  private PCB selectVictimFromSwapOut(PCB incoming) {
    ...
    // Tie-break: prefer swapping blocked workloads so READY can keep forward
    // progress.
    return blockedVictim;
  }
  ...
  private boolean shouldPreemptRunningForAdmission(PCB incoming, PCB running) {
    ...
    // Same urgency tier: allow admission to avoid NEW starvation when memory is
    // tight.
    return true;
  }
